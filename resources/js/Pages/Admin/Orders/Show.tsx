    ...
    return (
        <AdminLayout title={`Order ${order.order_number}`}>
            ...
            <div className="mb-5 flex items-center gap-3">
                <button onClick={() => router.get(route('admin.orders.index'))} className="text-xs text-ink-400 hover:text-ink-900">← Orders</button>
            ...
            <div className="grid gap-4 lg:grid-cols-3">
                        ...
                        <table className="w-full text-xs">
                            ...
                            <tbody className="divide-y divide-ink-200">
                                {order.items.map((i) => (
                                    <tr key={i.id}>
                                        <td className="py-2">
                                            <p className="text-ink-900 font-medium">{i.product_name}</p>
                                            {i.variant_label && <p className="text-ink-500">{i.variant_label}</p>}
                                            <p className="text-ink-400">{i.sku}</p>
                                        </td>
                                        <td className="py-2 text-right text-ink-500">{i.quantity}</td>
                                        <td className="py-2 text-right text-ink-500">{formatPHP(i.unit_price_cents)}</td>
                                        <td className="py-2 text-right text-ink-900 font-medium">{formatPHP(i.subtotal_cents)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                        ...
                        <Section title="Payments">
                            {order.payments.map((p) => (
                                <div key={p.id} className="flex items-center justify-between py-1.5 text-xs">
                                    <span className="text-ink-500">{p.type} · {p.status}</span>
                                    <span className="text-ink-900">{formatPHP(p.amount_cents)}</span>
                                </div>
                            ))}
                        </Section>
                    )}
                        ...
                        <Section title="Refunds">
                            {order.refunds.map((r) => (
                                <div key={r.id} className="flex items-center justify-between py-1.5 text-xs">
                                    <span className="text-ink-500">{r.reason ?? '—'} · {r.status}</span>
                                    <span className="text-red-600">-{formatPHP(r.amount_cents)}</span>
                                </div>
                            ))}
                        </Section>
                    )}
                ...
                <div className="space-y-4">
                    <Section title="Customer">
                        <p className="text-sm text-ink-900 font-medium">{order.user.name}</p>
                    ...
                    <Section title="Summary">
                        {[
                            ...
                            <div key={label} className="flex justify-between py-1 text-xs">
                                ...
                                <span className={label === 'Total' ? 'text-ink-900 font-bold' : 'text-ink-600'}>{value}</span>
                            </div>
                        ))}
                        {order.coupon_code && (
