...
export default function AdminLayout({ children, title }: { children: ReactNode; title?: string }) {
    ...
    return (
        /* No data-theme override — the entire admin responds to the user's theme toggle.
           bg-surface / bg-canvas / ink-* all switch correctly via CSS vars.             */
        <div className="flex min-h-screen bg-canvas text-ink-900">

            {/* ── Desktop sidebar ─────────────────────────────────────────────── */}
            <aside className="hidden w-56 shrink-0 flex-col border-r border-ink-200 bg-surface lg:flex">
                <div className="flex h-14 items-center px-5 border-b border-ink-200">
                    <Link href="/" className="text-sm font-bold tracking-widest text-brand uppercase">
                        Alarcon Admin
                    </Link>
                </div>
                <div className="flex-1 overflow-y-auto">{sidebar}</div>
                <div className="border-t border-ink-200 p-3 text-xs text-ink-500">
                    <p className="truncate font-medium text-ink-700">{auth.user?.name}</p>
                    <p className="truncate text-ink-400">{auth.user?.email}</p>
                </div>
            </aside>
            ...
            {mobileSidebarOpen && (
                <div className="fixed inset-0 z-40 lg:hidden">
                    ...
                    <aside className="absolute left-0 top-0 bottom-0 z-50 flex w-56 flex-col border-r border-ink-200 bg-surface">
                        <div className="flex h-14 items-center justify-between px-5 border-b border-ink-200">
                            <span className="text-sm font-bold tracking-widest text-brand uppercase">Admin</span>
                            <button
            ...
            <div className="flex flex-1 flex-col min-w-0">
                ...
                {/* Top bar */}
                <header className="flex h-14 items-center gap-3 border-b border-ink-200 bg-surface px-4 lg:px-6">
                    <button
                    ...
                    <h1 className="flex-1 text-sm font-semibold text-ink-900 truncate">{title ?? 'Admin'}</h1>
                    <ThemeToggle />
                    <Link href="/" className="text-xs text-ink-500 hover:text-ink-900">← Storefront</Link>
                </header>

                {/* Flash messages */}
                {flash?.success && (
                    <div className="mx-4 mt-4 rounded-lg border border-green-200 bg-green-50 px-4 py-2.5 text-sm text-green-700">
                        {flash.success}
                    </div>
                )}
                {flash?.error && (
                    <div className="mx-4 mt-4 rounded-lg border border-red-200 bg-red-50 px-4 py-2.5 text-sm text-red-700">
                        {flash.error}
                    </div>
                )}
